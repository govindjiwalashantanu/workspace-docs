...
### Gong Intelligence Pipeline + SFDC Sheet Enrichment + Sidebar — Jun 5, 2026
Full Gong-to-Calendar-to-Meeting data pipeline. **Bug fix:** `"Jun 4"` (no year) parsed as 2001 in V8 — calendar match window was wrong year. Fixed with ts-based date validation. **Gong data seeding:** `GET /api/gong/messages` now persists all parsed summaries as `gong_data_{ts}` entries on every Gong page load. **Calendar matching:** `calendar-sync/events` does Jaccard token scoring (≥40%, ±2 day window) against all stored `gong_data_*` — shows Gong Intelligence panel on calendar events automatically. **Gong tab:** `MeetingDetail` + `RecentMeetingsPage` show a Gong tab when meeting has `gongTs` (Gong analyze path), `calendarEventId` (calendar import), or any `date` NodeProp — uses `useGongDataForMeeting` hook → `GET /api/gong/match-for-meeting`. **Calendar import:** writes Gong NodeProperties at import time. **Auto-link:** 100% token match → auto-link via `useEffect` in `CalendarMatchBanner`. **Imported badge:** `importedEventIds` Set (dual check: calendarEventId + title+date) shows ✓ badge and blocks re-import; auto-refreshes Meetings list. **Sidebar:** top nav = Meetings → Notebook → Action Items → Calendar → Slack → Lucid → Gong; bottom = SFDC Sync → Pipeline → My Insights → Integrations → Ask SenSEi → Send Feedback → Settings. Integrations nav item navigates directly to Integrations tab; `settingsTab` in Zustand store, cleared on SettingsPage mount. **SFDC sheet:** POST handler fixed to create `sfdc-pipeline` + `sfdc-account` shadow nodes (was polluting notebook with `opportunity` nodes). Personal report column mapping (`detectFormat()` auto-detects). `enrichMatchingUserOpps` exported + called per-opp in POST + `POST /api/sync/sfdc-sheet/enrich-all` endpoint + notebook opp creation trigger. Cron MCP sheet sync removed — Apps Script 7am timer is primary sync path; cron runs `enrichAllLinkedOpps` only. 438 opps synced from personal SFDC report.

### Slack Monitor + Cross-User Security Hardening — Jun 3, 2026 (eve)
New `/dashboard#slack` page: 3-pane experience (channels sidebar with All / By account / Recent filters, Slack-style message stream with grouped messages and day dividers, context panel with linked opps + stats + recent participants). Add Channel modal (type tabs, live MCP search, atomic link). Routes: `monitored-channels`, `channel-messages`, `link-channel` (POST + DELETE). Channels also surface in the existing per-opp Slack tab. **Security**: closed two CRITICAL cross-user leaks. `/api/action-items` was filtering only by org (every SE saw every SE's items — Jason saw Shantanu's). `/api/slack/link-channel` had the same flaw on POST + DELETE (any SE could write to / unlink another SE's opps). Both fixed with `userId` scoping. POST link-channel now atomic via `$transaction`. Strict input validation on `channelId` (regex), `channelName` (max 120), `opportunityIds` (cap 25/batch). 22 new API contract + auth-scoping tests covering both bugs.

### Google Tasks Integration + Calendar Matching Rewrite — Jun 3, 2026
`Card` gains `googleTaskId`/`googleTaskListId`. Push-to-Tasks from CardModal + ActionItemModal footers with Google logo badge on Kanban tiles. Linked-first sort, inactive-user filter, SearchableSelect across all assignee pickers app-wide. Action items scoped to current user (was leaking across org). Auto-trigger full cascade when transcript manually saved (> 100 chars, deduped with `lastAutoAnalyzedTranscriptRef`). Due date synced to Google Tasks on create/update. Calendar event matching algorithm fully rewritten: 4 signals in priority order (contact match 97, domain Levenshtein 60–95, token overlap title 58–80, description 48–62); internal meeting guard (`INTERNAL_TITLE_RE`, all-internal attendee check); `'sensei'` added to VENDOR_WORDS; 673 stale cache entries cleared; `match-helpers.ts` shared lib consolidates both matching paths. Digest "Meetings Today" now reads `GoogleCalendarEventCache` instead of NotebookNode records (was always showing empty). TypeScript clean, 8 pre-existing failures unchanged.

### AI Intelligence Pipeline (Signals, DAG, RAG, Cascade Optimizations) — Jun 3, 2026
Meeting analysis extracts structured `meetingSignals` per meeting (stakeholders/sentiment, objections, buying signals, tech reqs, competitive, pain verbatim, open questions) in a single LLM pass. Action items auto-close with `resolvedEvidence` verbatim quote. `MeetingActionItem` gains `status`/`resolvedAt`/`resolvedEvidence` (migration applied). Transcripts auto-trigger full cascade on ingest + MCP `create_meeting`/`update_meeting`. `context-builder.ts` aggregates signals into `## Meeting Intelligence Signals` for CoM, 3 Whys, State, Elevator Pitch, BV. pgvector 0.8.0 on RDS: `TranscriptChunk` table + HNSW index; `text-embedding-3-small` embeddings; 2000-char chunks, 200-char overlap; cosine top-8 RAG injected for deals with 4+ meetings; 2,684 chunks seeded. LLM cascade 70–80% reduction: 5-min DB dedup lock, staleness gate (skip if `lastRun_com` newer than meeting), state lazy-refresh (skip if no new tech reqs), signals-first incremental CoM/3Whys (~8k vs ~290k tokens). Meeting Intelligence dashboard in Summary sub-tab: stakeholder cards, buying signals, objections, competitive chips, open questions, pain verbatim. Blue-tinted glass AI panel buttons.

### Google Workspace MCP + Calendar Sync — Jun 2–3, 2026
`GoogleCalendarEventCache` + `UserSyncState` Prisma models. `GET /api/sync/google-calendar` — full FY window sync, chunked fetch with per-chunk error tolerance. `POST /api/sync/google-tasks` + `GoogleTaskCache`. `SfdcOpportunityCache`. Calendar page: FY/Quarter/Month/Week/List views, search, agenda redesign, tasks right pane, email classification, contact auto-create on import. Daily cron + on-login background sync. LiteLLM NAT gateway (`52.206.25.250`) whitelisted at `llm.atko.ai` — ECS AI calls work in prod.

### MCP Settings Page Restructure + Prod URL Migration — May 17, 2026
Two-column MCP settings page in `McpSection` (SettingsPage.tsx): left column (440px) holds key management + available tools panel with dynamic tool count; right column is a full-width "wow moment" catalog with two 2-column grids. **Single MCP workflows:** 6 cards x 4 concrete SE examples (Slack, Gmail, Drive, Calendar, Zoom, Lucid). **Power workflows (multi-MCP):** 6 combo cards with MCP pill badges and accent-bordered examples: Full Meeting Lifecycle, Complete Deal Intelligence, POC Launch Kit, Stakeholder Intelligence, Weekly Pipeline Sweep, Executive QBR Prep. Dead `app/settings/mcp/page.tsx` tool count updated to 45. **Prod URL migrated:** `okta.se-n-sei.com` replaced with `sensei.oktademo.app` across all code files (MCP pages, setup page, smoke tests, E2E setup, DEPLOY.md, CLAUDE.md, privacy page, prod-readiness page). **Hero page:** "Art of the Possible" section added between Features and CTA -- 3 multi-MCP power workflow cards with link to `/setup`.

### 5 New MCP Tools + Two-Column MCP Settings — May 17, 2026
New tools: `search_transcripts` (keyword search with snippet context, `GET /api/search/transcripts`), `get_insights` (account AI insights, `GET /api/insights`), `list_action_items` (MeetingActionItem list with full context, `GET /api/action-items`), `create_contact` (`POST /api/notebook/${accountId}/contacts`), `get_health_score` (`GET /api/notebook/${opportunityId}/health-score`). Two new API routes: `app/api/search/transcripts/route.ts`, `app/api/action-items/route.ts`. MCP settings page initial two-column layout (tools in right column alongside What's Possible). 45 total tools.

### Event-Driven Agent Automation — May 17, 2026
Full event-driven analysis pipeline for Claude Code as MCP compute layer. **Phase 1 (DB + API routes):** 3 new Prisma models (`AnalysisDirtyFlag`, `UserAgentPreferences`, `AgentActivityLog`). 5 new API routes: `pending-queue` (content-gated queue), `clear-flag`, `preferences` (GET/PUT), `activity` (GET/POST), `set-flag` (dual auth: NextAuth + MCP Bearer). 16 new unit tests. **Phase 2 (dirty flags):** `TOOL_FLAG_MAP` + `setDirtyFlagForOpp` in `lib/mcp-tools.ts`; 9 handlers wired (`refresh_slack_intelligence`, `add_email_context`, `add_drive_doc`, `add_lucid_diagram`, `update_meeting`, `create_meeting`, `update_opportunity`, `append_presales_notes`, `find_replace_property`). Circular trigger guard: `write_analysis_result` never sets flags. **Phase 3 (skills):** Two Claude Code skills at `~/.claude/skills/`: `/analyze-queue` (drain queue, check prefs, prompt user, run in sequence, clear flags) and `/run-analysis <nodeId> <type>` (single opp, any type). **Phase 4 (UI):** `AgentSection` in Settings → "✦ Agent Automation" tab with preferences form (6 fields), recent activity log table. Agent Activity tab added to `AIJobBell.tsx` notification bell with tab switcher. **MCP tools:** `get_agent_queue`, `get_agent_preferences`, `update_agent_preferences` added to `lib/mcp-tools.ts`. Activity route enhanced to return `opportunityTitle` via node join. TypeScript clean.

### Restore AI Buttons with "Run via Claude Code" Modal Treatment — May 17, 2026
All AI action buttons restored app-wide. Clicking any button opens `MCPCommandModal` — shows a pre-filled copyable MCP command with node ID baked in, the AI prompt description, and the context being sent. `ClaudeCodeTooltip` `?` icon added to all AI panel headers to explain Claude Code to new SEs. `run_analysis` in `lib/mcp-tools.ts` extended from 4 to 14 types (added: elevator_pitch, bv, poc_extract, techqual, research, stakeholders, next_actions, meeting_analyze, meeting_prep, meeting_followups — split into NOTEBOOK_ROUTE_MAP vs AGENT_ROUTE_MAP). 12 buttons in OpportunityAIPanel, 3 in MeetingAIPanel, Research button back in AccountDetail. New components: `MCPCommandModal.tsx`, `ClaudeCodeTooltip.tsx`. Tests: 3116 passing, TypeScript clean.

### Remove All In-App AI Buttons / "Run via Claude Code" Treatment — May 17, 2026
LiteLLM proxy (`llm.atko.ai`) is IP-restricted to Okta internal network; ECS prod cannot reach it so all in-app AI trigger buttons were failing silently. Removed all AI trigger logic app-wide and replaced with static "RUN VIA CLAUDE CODE" hints referencing MCP commands. Files: `OpportunityAIPanel.tsx` (full rewrite — 4 MCP rows), `MeetingAIPanel.tsx` (full rewrite — persona selector kept), `app/admin/agents/page.tsx` (pure display, via-MCP badges), `AccountDetail.tsx` (Research button removed), `EmailContextPanel.tsx`, `SlackContextPanel.tsx`, `DriveContextPanel.tsx`, `AttachmentsPanel.tsx` (all Analyze buttons replaced with `ctx-analyze-mcp-hint` label). `MeetingAIPanel.test.tsx` updated (2 stale tests replaced). 3096 passing, TypeScript clean.

### MCP → API Route Architecture Refactor — May 7, 2026
All MCP tool cases in `lib/mcp-tools.ts` now route through `serviceFetch` instead of direct Prisma calls. Third auth path in `requireAuth()` accepts `x-service-token` + `x-service-user-id` + `x-service-org-id` headers (validated against `AGENT_CRON_SECRET`). New routes: `GET /api/notebook/[id]/children?type=...`, `POST /api/notebook/[id]/analysis-result`, `GET /api/notebook/[id]/poc-context`. Security gains: `verifyNotebookAccess`/`verifyNotebookEditAccess`/board-level `userId OR shares` enforced for all MCP reads and writes (was orgId-only scope). `STAGE_TO_COL` mapping in `create_card` fixes pre-existing Zod validation bug. 3074 passing.

### Sales Roles on Team Tab — May 5, 2026
`salesRole String?` added to `User` model. `inferSalesRole()` in `lib/auth.ts` auto-populates from Okta profile title on first sign-in (SE / AE / SE Manager / Area Sales Director / Director, Presales / AVP, Sales). `PATCH /api/organizations/current/members/:userId` allows any org member to update a teammate's role. Team tab in AccountDetail shows color-coded role badges; account owners get an inline `<select>` to change roles. `salesRole` exposed in share and members API responses — ready for insights filtering. `GET /api/notebook/:id/share` 500 bug fixed (Turbopack cached stale Prisma bundle — cleared `.next/`). `__tests__/e2e/team-tab.spec.ts` added: 8 E2E tests covering API layer (200/404/400) + UI (owner row, add-user, read-only state). 3010 unit tests passing, TypeScript clean.

### Leadership Insights Dashboard — May 5, 2026
`GET /api/admin/insights?period=quarter|fy` — superadmin-only; 8 metric sections: Adoption (DAU/WAU/PAU + 14-day sparkline), Meeting Intelligence (analysis rate), Pipeline (ARR, CoM rate, POC rate, health distribution), Action Items (overdue count, completion rate), AI Usage (runs by type, minutes saved), Competitive (top competitors), Pain Themes (top customer pain themes), Leaderboard (top 10 SEs by activeMs). All queries filter `isActive: true` to exclude E2E test accounts. `app/admin/insights/page.tsx` with period toggle + auto-refresh + Export PDF. CSS-only charts. 3010 passing.
